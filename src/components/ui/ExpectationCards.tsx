      ...
      {expectations.map((point, index) => {
        ...
        return (
          <FadeIn
            key={point.title}
            delay={index * 80}
            className={`group rounded-card border p-6 shadow-[0_1px_2px_rgb(7_26_58/0.04)] transition-all duration-300 ease-out hover:-translate-y-1 hover:shadow-float hover:shadow-brand-900/[0.08] ${
              tinted
          ...
          >
            <span
              className={`flex h-11 w-11 shrink-0 items-center justify-center rounded-control transition-transform duration-300 ease-out group-hover:scale-105 ${
                tinted ? "bg-white text-brand-500 shadow-raise" : "bg-gradient-to-br from-brand-50 to-brand-100 text-brand-500"
              }`}
            >
